...
class Config:
    ...
    # Realtime WLASL300 prediction tuning
    REALTIME_BASE_CONFIDENCE_SQUELCH = 0.65
    REALTIME_ADAPTIVE_MARGIN = 0.12
    REALTIME_ADAPTIVE_CONFIDENCE_FLOOR = 0.45
    REALTIME_STABILIZATION_WINDOW = 10
    REALTIME_STABILIZATION_MIN_COUNT = 6
    REALTIME_PEAK_HISTORY_WINDOW = 5
    REALTIME_PEAK_MIN_COUNT = 2
    REALTIME_PEAK_MARGIN = 0.10

    # Lower thresholds for signs that are repeatedly the top guess but hover below the
    # global confidence cutoff during live use.
    REALTIME_SIGN_CONFIDENCE_OVERRIDES = {
        "APPROVE": 0.58,
        "ARRIVE": 0.58,
        "BAD": 0.58,
        "BEFORE": 0.56,
    ...
    # Some signs reliably spike to the right answer only briefly before the user relaxes
    # into an end pose that the model confuses with another class. Allow a short,
    # high-confidence burst to stabilize them earlier.
    REALTIME_PEAK_SIGN_CONFIDENCE_OVERRIDES = {
        "ARRIVE": 0.72,
        "CATCH": 0.68,
        "CHRISTMAS": 0.62,
        "HOPE": 0.72,
    ...
    # Suppress idle false positives for signs that should only be emitted while the hands
    # are actively moving through the sign.
    REALTIME_SIGN_MOTION_REQUIREMENTS = {
        "AGAIN": 0.016,
        "ARRIVE": 0.016,
        "CATCH": 0.016,
        "CEREAL": 0.016,
    ...
    # If these rivals are nearly tied, hold output until the model becomes clearer.
    REALTIME_CONFUSION_PAIRS = {
        "AGAIN": {"DOCTOR"},
        "APPROVE": {"DRAW"},
        "ARRIVE": {"BABY"},
        "BAD": {"SCHOOL"},
